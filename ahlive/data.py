...
class Data(Easing, Animation, Configuration):
    ...
    chart = param.ClassSelector(class_=Iterable, doc=f"Type of plot; {CHARTS['all']}", precedence=PRECEDENCES["common"])
    preset = param.ObjectSelector(
        objects=list(chain(*PRESETS.values())), doc=f"Chart preset; {PRESETS}", precedence=PRECEDENCES["common"]
    )
    state_labels = param.ClassSelector(
        class_=(Iterable,), doc="Dynamic label per state (bottom right)", precedence=PRECEDENCES["common"]
    )
    inline_labels = param.ClassSelector(
        class_=(Iterable,),
        doc="Dynamic label per item per state (item location)", precedence=PRECEDENCES["common"]
    )
    label = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="Legend label for each item", precedence=PRECEDENCES["common"]
    )
    group = param.String(doc="Group label for multiple items", precedence=PRECEDENCES["misc"])

    title = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="Title label (outer top left)", precedence=PRECEDENCES["label"]
    )
    xlabel = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="X-axis label (bottom center)", precedence=PRECEDENCES["label"]
    )
    ylabel = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="Y-axis label (left center", precedence=PRECEDENCES["label"]
    )
    subtitle = param.ClassSelector(
        class_=(int, float, str),
        allow_None=True,
        doc="Subtitle label (outer top right)", precedence=PRECEDENCES["sub_label"]
    )
    note = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="Note label (bottom left)", precedence=PRECEDENCES["sub_label"]
    )
    caption = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="Caption label (outer left)", precedence=PRECEDENCES["sub_label"]
    )
    xlims = param.ClassSelector(class_=Iterable, doc="Limits for the x-axis", precedence=PRECEDENCES["limit"])
    ylims = param.ClassSelector(class_=Iterable, doc="Limits for the y-axis", precedence=PRECEDENCES["limit"])
    xlim0s = param.ClassSelector(
        class_=(Iterable, int, float),
        doc="Limits for the left bounds of the x-axis", precedence=PRECEDENCES["limit"]
    )
    xlim1s = param.ClassSelector(
        class_=(Iterable, int, float),
        doc="Limits for the right bounds of the x-axis", precedence=PRECEDENCES["limit"]
    )
    ylim0s = param.ClassSelector(
        class_=(Iterable, int, float),
        doc="Limits for the bottom bounds of the y-axis", precedence=PRECEDENCES["limit"]
    )
    ylim1s = param.ClassSelector(
        class_=(Iterable, int, float),
        doc="Limits for the top bounds of the y-axis", precedence=PRECEDENCES["limit"]
    )
    xmargins = param.Number(doc="Margins on the x-axis; ranges from 0-1", precedence=PRECEDENCES["limit"])
    ymargins = param.Number(doc="Margins on the y-axis; ranges from 0-1", precedence=PRECEDENCES["limit"])

    xticks = param.ClassSelector(class_=(Iterable,), doc="X-axis tick locations", precedence=PRECEDENCES["style"])
    yticks = param.ClassSelector(class_=(Iterable,), doc="Y-axis tick locations", precedence=PRECEDENCES["style"])

    legend = param.ObjectSelector(objects=OPTIONS["legend"], doc="Legend location", precedence=PRECEDENCES["style"])
    grid = param.ObjectSelector(default=True, objects=OPTIONS["grid"], doc="Grid type", precedence=PRECEDENCES["style"])
    style = param.ObjectSelector(
        objects=OPTIONS["style"], doc=f"Chart style; {OPTIONS['style']}", precedence=PRECEDENCES["style"]
    )

    hooks = param.HookList(
        doc="List of customization functions to apply; "
        "function must contain fig and ax as arguments", precedence=PRECEDENCES["misc"]
    )

    rowcol = param.NumericTuple(
        default=(1, 1), length=2, doc="Subplot location as (row, column)", precedence=PRECEDENCES["misc"]
    )
    ...
    configurables = param.Dict(
        ...
        constant=True, precedence=PRECEDENCES["attr"]
    )
    attrs = param.Dict(doc="Attributes of the first dataset", constant=True, precedence=PRECEDENCES["attr"])
...
class GeographicData(Data):
    ...
    crs = CartopyCRS(doc="The coordinate reference system to project from", precedence=PRECEDENCES["geo"])
    projection = CartopyCRS(doc="The coordinate reference system to project to", precedence=PRECEDENCES["geo"])
    central_lon = param.ClassSelector(
        class_=(Iterable, int, float), doc="Longitude to center the map on", precedence=PRECEDENCES["geo"]
    )

    borders = CartopyFeature(doc="Whether to show borders", precedence=PRECEDENCES["geo"])
    coastline = CartopyFeature(doc="Whether to show coastlines", precedence=PRECEDENCES["geo"])
    land = CartopyFeature(doc="Whether to show land surfaces", precedence=PRECEDENCES["geo"])
    ocean = CartopyFeature(doc="Whether to show ocean surfaces", precedence=PRECEDENCES["geo"])
    ...
    worldwide = param.Boolean(doc="Whether to view globally", precedence=PRECEDENCES["geo"])

    tiles = CartopyTiles(doc="Whether to show web tiles", precedence=PRECEDENCES["geo"])
    zoom = param.Integer(default=None, bounds=(0, 25), doc="Zoom level of tiles", precedence=PRECEDENCES["geo"])
...
class ColorArray(param.Parameterized):
    ...
    cs = param.ClassSelector(
        class_=(Iterable,), doc="Array to be mapped to the colorbar", precedence=PRECEDENCES["common"]
    )

    cticks = param.ClassSelector(class_=(Iterable,), doc="Colorbar tick locations", precedence=PRECEDENCES["limit"])
    ctick_labels = param.ClassSelector(class_=(Iterable,), doc="Colorbar tick labels", precedence=PRECEDENCES["sub_label"])
    colorbar = param.Boolean(default=None, doc="Whether to show colorbar", precedence=PRECEDENCES["style"])
    clabel = param.ClassSelector(
        class_=(int, float, str), allow_None=True, doc="Colorbar label", precedence=PRECEDENCES["label"]
    )
...
class Array(GeographicData, ReferenceArray, ColorArray, RemarkArray):
    ...
    xs = param.ClassSelector(class_=(Iterable,), doc="Array to be mapped to the x-axis", precedence=PRECEDENCES["xy"])
    ys = param.ClassSelector(class_=(Iterable,), doc="Array to be mapped to the y-axis", precedence=PRECEDENCES["xy"])
...
class Array2D(GeographicData, ReferenceArray, ColorArray, RemarkArray):
    ...
    inline_xs = param.ClassSelector(
        class_=(Iterable, int, float), doc="Inline label's x locations", precedence=PRECEDENCES["sub_label"]
    )
    inline_ys = param.ClassSelector(
        class_=(Iterable, int, float), doc="Inline label's y locations", precedence=PRECEDENCES["sub_label"]
    )
...
class DataStructure(param.Parameterized):
    ...
    join = param.ObjectSelector(
        objects=ITEMS["join"], doc=f"Method to join; {ITEMS['join']}", precedence=PRECEDENCES["common"]
    )
...
class Dataset(Array2D, DataStructure):
    ...
    ds = param.ClassSelector(class_=(xr.Dataset,), doc="Xarray Dataset", precedence=PRECEDENCES["data"])
...
class Reference(GeographicData):
    ...
    chart = param.ObjectSelector(
        objects=CHARTS["ref"], doc=f"Type of chart; {CHARTS['ref']}", precedence=PRECEDENCES["common"]
    )

    x0s = param.ClassSelector(
        class_=(Iterable,), doc="Array to be mapped to lower x-axis", precedence=PRECEDENCES["xy"]
    )
    x1s = param.ClassSelector(
        class_=(Iterable,), doc="Array to be mapped to upper x-axis", precedence=PRECEDENCES["xy"]
    )
    y0s = param.ClassSelector(
        class_=(Iterable,), doc="Array to be mapped to lower y-axis", precedence=PRECEDENCES["xy"]
    )
    y1s = param.ClassSelector(
        class_=(Iterable,), doc="Array to be mapped to lower y-axis", precedence=PRECEDENCES["xy"]
    )
    inline_locs = param.ClassSelector(
        class_=(Iterable, int, float), doc="Inline label's other axis' location", precedence=PRECEDENCES["label"]
    )
